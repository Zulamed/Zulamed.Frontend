...
export async function getVideosBySubscriptions(subscriptions: boolean, fetch: FetchCallbackType = originalFetch): Promise<Response> {
    try {
        const result = await fetch(`${PUBLIC_BACKEND_URL}/video?subscriptions=${subscriptions}`);
        if (!result.ok){
